...
- Use a municipal, county, state, federal, transit-agency, school-district, or other primary public source.
- Add the source to the same content section as the claim it supports.
- Give every source a plain-language title, a credential-free HTTPS URL, and an accessed date.
- Recheck volatile facts such as prices, taxes, school assignments, flood maps, transit schedules, and percentages immediately before publication.
- Do not convert a source's promotional language into the site's own claim.
- Keep the existing page-level community-information note. Source blocks support facts; they do not need to repeat legal copy in every card.

Editor source-block shape:

```json
{
  "tag": "SOURCE",
  "text": "Gloucester County Municipalities",
  "href": "https://www.gloucestercountynj.gov/831/Municipalities",
  "accessed": "2026-07-18"
}
```

## County Introduction Drafts
